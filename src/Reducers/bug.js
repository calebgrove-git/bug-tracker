...
export function fetchBugs(company) {
    return async dispatch => {
      try {
        const response = await fetch(`https://secure-journey-81702.herokuapp.com/api/bugs/get/${company}`)
        const data = await response.json()
        let sorted
        if(data){ sorted = data.sort((a,b)=>{
...
export function completeBug(bug) {
      return function dispatch(){
              return fetch(`https://secure-journey-81702.herokuapp.com/bugs/${bug.id}/complete`,{
                  method: 'PATCH',
                  mode: 'cors',
                  headers:{
                      'Content-Type':'application/json'
...
export function deleteBug(bug) {
            return function dispatch(){
                    return fetch(`https://secure-journey-81702.herokuapp.com/api/bugs/${bug.id}`,{
                        method: 'DELETE',
                        mode: 'cors',
                        headers:{
                            'Content-Type':'application/json'
